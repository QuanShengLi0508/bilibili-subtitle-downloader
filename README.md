...
选择「网页/知乎文本」模式，粘贴知乎回答或专栏链接，点「直接获取文字」。程序会请求知乎正文接口，把正文直接保存为 TXT。

首次使用知乎需要登录一次：点击「打开知乎登录」，登录后在浏览器开发者工具里复制请求头中的整串 `Cookie`，粘贴到软件里点「保存 Cookie」。Cookie 会保存在本机 `C:\Users\<用户名>\.zhihu-cookies.json`。

其他网页仍可点「打开链接」，复制正文后点「保存剪贴板为 TXT」。
